...
void timing(void)  /* Timing function. */{
    ...
    nextEventType = 1; //Al final debe quedar en 1 en caso de que el proximo evento sea un arribo, 2: si es la salida de un servicio
	float lowerTime = timeNextEvent[1];
	
    /* Determine the event type of the next event to occur. */
    
    int i=0;
    while(i<SERVERS-1){
		if(lowerTime>nextDepartures[i]){
			lowerTime = nextDepartures[i];
			nextEventType=2;
		}
		i++;
	}
...
void arrive(void)  /* Arrival event function. */{
    ...
    if (freeServers==0) {
        ...
        /* All servers are busy, so increment number of customers in queue. */
        ...
        nextDepartures[abs(freeServers-SERVERS)]=simulationTime + generateNextDeparture() ;;
        clientsTime[totalClients][2]=nextDepartures[abs(freeServers-SERVERS)];
...
void depart(void)  /* Departure event function. */{
    ...
    if (elementsInQueue == 0) {
		...
		nextDepartures[abs(freeServers-SERVERS)]=1.0e+30;
		freeServers++;
		printf("	Empty queue\n");
        ...
        nextDepartures[abs(freeServers-SERVERS)]=simulationTime+generateNextDeparture();
        clientsTime[currentClient][2]=nextDepartures[abs(freeServers-SERVERS)];
